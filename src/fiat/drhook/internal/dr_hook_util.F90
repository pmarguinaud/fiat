...
CHARACTER*128, ALLOCATABLE, SAVE :: CL_NVTX_LIST (:)
...
IF (II_DRNVTX == 0) THEN
  ...
  IF (CL_NVTX == '1') THEN
    II_DRNVTX = +1
    IL = LINECOUNT ("nvtx.txt")
    IF (IL > 0) THEN
      ALLOCATE (CL_NVTX_LIST (IL))
      CALL READFILE ("nvtx.txt", CL_NVTX_LIST)
    ENDIF
  ELSE
    II_DRNVTX = -1
  ENDIF
...
IF (II_DRNVTX == 1) THEN
  IF (ALLOCATED (CL_NVTX_LIST)) THEN
    IF (ALL (CL_NVTX_LIST /= CDNAME)) THEN
      GOTO 100
    ENDIF
  ENDIF
  IF (KCASE == 0) THEN
...
INTEGER FUNCTION LINECOUNT (CDFILE)
...
OPEN (NEWUNIT=IUNIT, FILE=TRIM (CDFILE), FORM='FORMATTED', ERR=999)

DO 
  READ (IUNIT, *, END=999, ERR=999) CLLINE
...
SUBROUTINE READFILE (CDFILE, CDLINE)

CHARACTER(LEN=*) :: CDFILE
CHARACTER(LEN=*) :: CDLINE (:)

INTEGER :: IUNIT, IL

OPEN (NEWUNIT=IUNIT, FILE=TRIM (CDFILE), FORM='FORMATTED')

DO IL = 1, SIZE (CDLINE)
  READ (IUNIT, *) CDLINE (IL)
ENDDO
