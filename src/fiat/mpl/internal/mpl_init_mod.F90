...
CHARACTER(LEN=32)  :: CL_MPL_HEAP_SIZE
INTEGER*8          :: IL_MPL_HEAP_SIZE
...
CALL GET_ENVIRONMENT_VARIABLE('MPL_HEAP_SIZE',CL_MPL_HEAP_SIZE)

IF (CL_MPL_HEAP_SIZE /= '') THEN
  ILEN = LEN_TRIM (CL_MPL_HEAP_SIZE)
  IF (CL_MPL_HEAP_SIZE (ILEN-1:ILEN) == 'MB') THEN
    READ (CL_MPL_HEAP_SIZE (1:ILEN-2), *) IL_MPL_HEAP_SIZE
    IL_MPL_HEAP_SIZE = IL_MPL_HEAP_SIZE * 1024_8**2
  ELSEIF (CL_MPL_HEAP_SIZE (ILEN-1:ILEN) == 'GB') THEN
    READ (CL_MPL_HEAP_SIZE (1:ILEN-2), *) IL_MPL_HEAP_SIZE
    IL_MPL_HEAP_SIZE = IL_MPL_HEAP_SIZE * 1024_8**3
  ELSE
    CALL ABOR1 ('MPL_HEAP_SIZE : UNEXPECTED VALUE '//TRIM (CL_MPL_HEAP_SIZE))
  ENDIF 
  WRITE (0, *) " MPL_HEAP_SIZE = ", CL_MPL_HEAP_SIZE
  CALL MPL_HEAP%INIT (IL_MPL_HEAP_SIZE)
ENDIF
